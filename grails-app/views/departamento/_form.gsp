...
<div class="fieldcontain ${hasErrors(bean: departamentoInstance, field: 'chefe', 'error')} ">
	...
	<g:select id="chefe" name="chefe.id" from="${alocadorhorarios.Professor.list()}" optionKey="id" value="${departamentoInstance?.chefe?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>

<div class="fieldcontain ${hasErrors(bean: departamentoInstance, field: 'subchefe', 'error')} ">
	<label for="subchefe">
		<g:message code="departamento.subchefe.label" default="Subchefe" />
